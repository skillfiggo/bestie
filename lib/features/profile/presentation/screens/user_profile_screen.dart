  ...
  Widget build(BuildContext context) {
    ...
    return Scaffold(
      ...
      bottomNavigationBar: profileAsync.when(
        data: (profile) {
          if (profile == null || profile.role == 'system') return null;
          return Container(
                              ...
                              if (profile.locationName.isNotEmpty) ...[ 
                                ...
                                Builder(builder: (context) {
                                  ...
                                  return Row(
                                    mainAxisAlignment: MainAxisAlignment.spaceBetween,
                                    children: [
                                      Expanded(child: _buildStat(statsAsync.isLoading ? '...' : '${stats.followerCount}', 'Followers')),
                                      Expanded(child: _buildStat(statsAsync.isLoading ? '...' : '${stats.followingCount}', 'Following')),
                                      _buildStat(statsAsync.isLoading ? '...' : '${stats.friendsCount}', 'Friends'),
                                      const SizedBox(width: 24),
                                      _buildStat(statsAsync.isLoading ? '...' : '${stats.bestiesCount}', 'Bestie'),
                                      const Spacer(),
                                    ],
                                  );
                                }),
